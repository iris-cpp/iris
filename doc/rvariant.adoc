...
= rvariantlink:https://github.com/iris-cpp/iris[image:https://img.shields.io/badge/GitHub-iris-purple?logo=github[GitHub], window="_blank"]
Yaito Kakeyama; Nana Sakisaka
:license-url: https://github.com/iris-cpp/iris/blob/main/LICENSE
...
:recursive_wrapper_alloca: pass:macros[xref:#rvariant.recursive[recursive_wrapper_alloca]]
...
struct BinaryExpr;
using Expr = iris::<<rvariant.rvariant,rvariant>><int, double, iris::<<rvariant.recursive,recursive_wrapper>><BinaryExpr>>;
enum class Op;
...
using AB  = iris::<<rvariant.rvariant,rvariant>><A, B>;
...
A& a = iris::<<rvariant.get,get>><0>(ab);
A& a = iris::get<A>(ab);
C& c = iris::get<C>(ab);     // throws {bad-variant-access}

A* a = iris::<<rvariant.get,get_if>><0>(&ab);
...
iris::<<rvariant.visit,visit>>(visitor, ab); // function visit
----
...
static_assert(iris::<<rvariant.syn,variant_size_v>><AB> == 2);
static_assert(std::same_as<iris::<<rvariant.helper,variant_alternative_t>><0, AB>, A>);
static_assert(iris::<<rvariant.get,holds_alternative>><A>(ab));
...
// <<rvariant.io,I/O support>>
{
    ...
    std::println("foo{}bar", iris::format_by(v_fmt, V(42)); // prints pass:quotes[`foo0042bar`]
    std::println("foo{}bar", iris::format_by(v_fmt, V(3.14)); // prints pass:quotes[`foo3.1bar`]
}
----
...
[[rvariant.syn]]
== Header <iris/rvariant.hpp> synopsis [.slug]##<<rvariant.syn,[rvariant.syn]>>##
...
template<class T> using unwrap_recursive_t = {see-below};
...
* [.underline]#Let `T` denote an arbitrary type. For the template parameter of `rvariant`, if a user provides both `T` and any instantiation of `{recursive_wrapper}` or `{recursive_wrapper_alloca}` that has the `value_type` of `T`, the program is ill-formed.#

* [.underline]#Let `T` denote an arbitrary type. For the template parameter of `rvariant`, if a user provides multiple different instantiations of `{recursive_wrapper_alloca}` such that the first template parameter is `T`, the program is ill-formed.#
...
* If `{recursive_wrapper}<T~_i_~>` or `{recursive_wrapper_alloca}<T~_i_~, A>` (with any type `A`) occurs anywhere in `Ts\...`, let `VT~_i_~` denote that type; otherwise, let `VT~_i_~` denote `T~_i_~`. Let `U~_j_~` denote the _j_^th^ type of the template parameter pack having the name `Us` on each flexibility-related functions. [.underline]#The _corresponding alternative_ for `rvariant` is the first type for which `std::is_same_v<{unwrap_recursive_t}<VT~_i_~>, {unwrap_recursive_t}<U~_j_~>>` is `true`#.
...
* [.candidate]#5)# *_Mandates:_* [.underline]#`T` is not a specialization of `{recursive_wrapper}` or `{recursive_wrapper_alloca}`#.
+
*_Constraints:_*
+
--
...
* [.candidate]#6)# *_Mandates:_* [.underline]#`T` is not a specialization of `{recursive_wrapper}` or `{recursive_wrapper_alloca}`#.
+
*_Constraints:_*
+
--
...
* [.candidate]#1)# *_Mandates:_* [.underline]#`T` is not a specialization of `{recursive_wrapper}` or `{recursive_wrapper_alloca}`.#
+
...
* [.candidate]#2)# *_Mandates:_* [.underline]#`T` is not a specialization of `{recursive_wrapper}` or `{recursive_wrapper_alloca}`.#
+
...
+
*_Remarks:_* [.underline]#This function is defined as deleted if `T` is a specialization of `{recursive_wrapper}` or `{recursive_wrapper_alloca}`.#
...
+
*_Returns:_* [.underline]#`o`, where `o` denotes a reference to the object stored in `v`, if the type of the expression's receiver is a specialization of `{recursive_wrapper}` or `{recursive_wrapper_alloca}`; otherwise, returns `{unwrap_recursive}(o)`#.
...
* [.candidate]#3-4)# Equivalent to the `std::variant` counterpart ^https://eel.is/c++draft/variant.visit[[spec\]]^, except that it forwards to `iris::visit` instead of `std::visit`.
...
template<class... Ts>
struct hash<::iris::rvariant<Ts...>>;pass:quotes[[.candidate\]#// 1#]

template<class T>
struct hash<::iris::recursive_wrapper<T>>;pass:quotes[[.candidate\]#// 2#]

template<class T, class Allocator>
struct hash<::iris::recursive_wrapper_alloca<T, Allocator>>;pass:quotes[[.candidate\]#// 3#]
...
template<class T>
/* constexpr */ std::size_t hash_value(recursive_wrapper<T> const& rw);pass:quotes[[.candidate\]#// 2#]

template<class T, class Allocator>
/* constexpr */ std::size_t hash_value(recursive_wrapper_alloca<T, Allocator> const& rw);pass:quotes[[.candidate\]#// 3#]
...
* [.candidate]#2)# *_Effects:_* Equivalent to `std::hash<recursive_wrapper<T>>{}(rw)`.

* [.candidate]#3)# *_Effects:_* Equivalent to `std::hash<recursive_wrapper_alloca<T, Allocator>>{}(rw)`.
--


[[rvariant.io]]
== I/O [.slug]##<<rvariant.io,[rvariant.io]>>##
I/O components are _not_ included by the global convenience header (`<iris/rvariant.hpp>`).
...
* The specialization `std::formatter<::iris::rvariant<Ts\...>, charT>` (for arbitrary `charT`) is enabled if and only if `std::formattable<{unwrap_recursive_t}<Ts~_i_~>, charT>` is `true` for all _i_, with the following characteristics:
+
[none]
...
[,cpp,subs="+macros,+attributes"]
----
std::println("{}", iris::rvariant<int, double>(42)); // prints pass:quotes[`42`]
----
...
Class template `recursive_wrapper` behaves like `recursive_wrapper_alloca` instantiated with `std::allocator<T>`, except that the allocator-related member functions (that is, member functions that take allocator-specific arguments) are omitted.
(_Note:_ this is a QoL feature to discard the rarely used `std::allocator<T>` type parameter from diagnostic messages; see https://github.com/iris-cpp/iris/issues/43[iris-cpp/iris#43].)
...
class recursive_wrapper_alloca
{
  ...
  constexpr pass:quotes[[.underline\]#/* not explicit */#] recursive_wrapper_alloca();

  template<class U = T>
  constexpr pass:quotes[[.underline\]#/* not explicit */#] recursive_wrapper_alloca(U&& x);
};
...
WARNING: `recursive_wrapper` and `recursive_wrapper_alloca` are *not* type alias of `std::indirect` and do *not* publicly derive from it.

NOTE: Although `std::indirect` is a {cpp}26 feature, `recursive_wrapper` and `recursive_wrapper_alloca` can be used in {cpp}23.
...
[,cpp,subs="+macros,+attributes"]
----
constexpr pass:quotes[[.underline\]#/* not explicit */#] recursive_wrapper_alloca();pass:quotes[[.candidate\]#// 1#]

template<class U = T>
constexpr pass:quotes[[.underline\]#/* not explicit */#] recursive_wrapper_alloca(U&& u);pass:quotes[[.candidate\]#// 2#]
----
...
+
_Note 2:_ It is currently unknown whether the recursive instantiation scenario described in _Note 1_ can be technically avoided without depending on the fragile mutual-exclusiveness on `std::is_constructible` and `std::is_convertible`. If you are aware of any technical insights, please contact us at https://github.com/iris-cpp/iris/issues[iris-cpp/iris].
--
+
...
[.candidates]
* [.candidate]#{empty}# Let `U` be `std::remove_cvref_t<T>`, and let `V` be `U::value_type` if `U` is a specialization of `{recursive_wrapper}` or `{recursive_wrapper_alloca}`, and `U` otherwise. Denotes the type obtained from `T` by replacing `U` with `V`, preserving the const-qualification and the reference (if any) of `T`.
* [.candidate]#{empty}# *Note:* For example, `unwrap_recursive_t<{recursive_wrapper}<X> const&>` denotes `X const&`. If `T` is a reference type, `unwrap_recursive_t<T>` denotes the same type as `decltype({unwrap_recursive}(std::declval<T>()))`. If `T` is not a reference type, `unwrap_recursive_t<T>` is also not a reference type, so that it can be further qualified (e.g. `unwrap_recursive_t<T> const&`).
...
[.candidates]
* [.candidate]#{empty}# *_Returns:_* `*o`, if cv-unqualified non-reference type for `T` is a specialization of `{recursive_wrapper}` or `{recursive_wrapper_alloca}`. Otherwise, returns `o`.
+
...
[[license]]
== License
This library is distributed under the https://github.com/iris-cpp/iris/blob/main/LICENSE[MIT License].
