...
[NOTE]
This library is intended for future submission to the Boost Libraries. Until then, the namespace is documented as `temp_ns` to indicate that the name is not yet stable. The current implementation temporarily uses the namespace `iris`, which originates from the prototype codebase.
