...
struct type_bases<std::type_identity<Ts>...> : std::type_identity<Ts>...
{
    template<template<class...> class TT>
    using rebind = TT<Ts...>;
};
...
struct type_bases_push_back_impl;

template<class... Ts, class T>
struct type_bases_push_back_impl<type_bases<std::type_identity<Ts>...>, T>
{
    using type = type_bases<std::type_identity<Ts>..., std::type_identity<T>>;
};

} // detail

template<class Bases, class T>
using type_bases_push_back = detail::type_bases_push_back_impl<Bases, T>::type;

// --------------------------------------------------

template<class... Ts>
using identity_bases = type_bases<std::type_identity<Ts>...>;

template<class T, class BasesT>
concept has_identity_base = requires(BasesT* bases) {
